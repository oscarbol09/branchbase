...
func TestServerLifecycle(t *testing.T) {
	...
	if srv.Listener() != nil {
		t.Errorf("expected srv.Listener() to be nil after Stop")
	}
}
...
func TestServerConcurrentStartStop(t *testing.T) {
	// Repeatedly start and stop concurrently to stress synchronization with -race
	for i := 0; i < 20; i++ {
		cfg := config.DefaultConfig()
		cfg.Proxy.ListenPort = 0
		cfg.Connection.Host = "127.0.0.1"
...
func TestServerStopBeforeStart(t *testing.T) {
	...
	// Start should fail cleanly without leaking an orphaned listener
	err := srv.Start(context.Background())
	if err == nil {
		t.Fatal("expected Start() after Stop() to return an error")
	}
	if srv.Listener() != nil {
		t.Fatal("expected Listener() to be nil when Start() fails after Stop()")
	}
}
